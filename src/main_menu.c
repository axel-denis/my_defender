...
void change_button_text(button *buttons, env_t *env, menu_t *menu)
{
    if (env->langue[0] == 'E') {
    ...
    menu->score = scoreboard_create(env);
    for (int i = 0; i < 4; i++)
...
int main_menu_button_gestion(SFWIN win, menu_t *menu, object mouse, env_t *env)
{
    for (int i = 0; i < 4; i++)
        display_button(win, &(menu->buttons[i]), env->keys);
    update_mouse_cursor(win, mouse, env->tempo);
    if (is_pressed(menu->buttons[0], win, env->keys) == sfTrue) {
        launch_game(win, mouse, env);
        menu->score = scoreboard_create(env);
    }
    if (is_pressed(menu->buttons[1], win, env->keys) == sfTrue) {
        options(win, mouse, env->keys, env);
        change_button_text(menu->buttons, env, menu);
        for (int i = 0; i < 4; i++) {
            sfSound_setVolume(menu->buttons[i].click, env->volume);
            sfSound_setVolume(menu->buttons[i].hover, env->volume / 1.4);
        }
    }
    if (is_pressed(menu->buttons[3], win, env->keys) == sfTrue)
        return 0;
    if (is_pressed(menu->buttons[2], win, env->keys) == sfTrue)
...
void main_menu(sfRenderWindow *window, object mouse, env_t *env)
{
    ...
    while (sfRenderWindow_isOpen(window) && open) {
        ...
        open = main_menu_button_gestion(window, &menu, mouse, env);
